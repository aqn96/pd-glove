...
- **Post-coursework research direction:** the sharper claim is **per-finger sensing for PD vs essential tremor**, not PD vs healthy. The discriminating sign (pill-rolling) is finger-level, which is exactly what wrist sensors average away, and iTex named that gap without filling it. Two public datasets drive it: **PADS** (only source of essential tremor) and **mPower** (only source of paired multimodal data, so the fusion head can be pretrained there). Voice, phone-in-pocket gait, and screen tapping are supporting modalities. Architecture diagram, dataset choices, study design, staging, and risks in [`docs/research-direction.md`](docs/research-direction.md).
