...
- **Post-coursework research direction:** a multimodal low-cost home PD assessment system, with **PD vs essential tremor** as the validity check rather than a bonus (ET is more common than PD, so a home system that calls all tremor Parkinson's misclassifies much of its real user base). The contribution is narrower than "we did multimodal": **per-finger inertial sensing captures inter-digit phase**, an EMG-free proxy for the established alternating-vs-synchronous sign that distinguishes the two conditions. Five scoring channels, late fusion, fully on-device. Two datasets: **PADS** (only ET source anywhere) and **mPower** (only paired multimodal source). Diagrams, sourcing, study design, staging, risks, and open decisions in [`docs/research-direction.md`](docs/research-direction.md).
